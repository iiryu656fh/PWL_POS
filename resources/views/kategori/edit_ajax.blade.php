...
@else
    <form action="{{ url('/kategori/' . $kategori->kategori_id.'/update_ajax') }}" method="post">
        @csrf
        @method('PUT')
    ...
    <script>
        $(document).ready(function() {
            $('#form-edit').validate({
                rules: {
                    kategori_kode: {required: true, maxlength: 3, pattern: /^[A-Z]+$/},
                    kategori_nama: {required: true, minlength: 3, maxlength: 50, pattern: /^[a-zA-Z]+$/}
                },
